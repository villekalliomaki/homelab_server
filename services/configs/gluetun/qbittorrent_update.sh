...
qbittorrent_login_res=$(curl \
                            ...
                            -o /dev/null \
                            -w "%{http_code}" \
                            -c "{{ dirs_container_volume_subdirectories.results[4].path }}/qbittorrent_cookies.txt" \
...
qbittorrent_res=$(curl \
                        -s \
                        -b "{{ dirs_container_volume_subdirectories.results[4].path }}/qbittorrent_cookies.txt" \
                        -o /dev/null \
                        -w "%{http_code}" \
